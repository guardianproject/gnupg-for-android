...
/**
   ...
   A signature doesn't stand alone. It is always bound to a key.
   So if you want to see a signature, you first have to get a key 
   via the GnuPGContext.
   <p>
   This class accesses directly the corresponding structure "under the hood"
   and stores no data in java members. This means, that every call of
   a method also results in a native call to access gpgme memory.
...
public class GnuPGSignature extends GnuPGPeer{
    ...
    /**
       ...
       when the lib browses thru the linked list of signatures of each key.
